...
// Orden en DOM:      P5, P8, P7, P6, P1, P4, P9, P2
// Índice en oculto:   3,  6,  5,  4,  0,  2,  7,  1
// ------------------------------------------------------------
const CARD_INDEX_MAP = [3, 6, 5, 4, 0, 2, 7, 1];
...
function abrirModalNuevo(cardIndex) {
  ...
  // --- Detectar si el link es un video (demo) o código ---
  const esDemo = linkHref.includes("youtube.com") || linkHref.includes("youtu.be") || linkHref.includes("vimeo.com");

  // --- Detectar idioma actual ---
  const idioma = localStorage.getItem("idioma") || "es";

  // --- Texto e ícono del botón según tipo y idioma ---
  const textoBoton = esDemo
    ? (idioma === "en" ? "Watch demo" : "Ver demo")
    : (idioma === "en" ? "View code"  : "Ver código");

  const iconoBoton = esDemo
  ...
  // --- Textos del modal según idioma ---
  const textoBadgePro      = idioma === "en" ? "Professional Internship" : "Práctica Profesional";
  const textoBadgePersonal = idioma === "en" ? "Personal Project"        : "Proyecto Personal";
  const textoCertificado   = idioma === "en" ? "Certificate"             : "Certificado";
  const textoConfidencial  = idioma === "en" ? "Confidential code"       : "Código confidencial";
  const textoCerrar        = idioma === "en" ? "Close"                   : "Cerrar";
    ...
    <div class="mpn-panel" role="dialog" aria-modal="true">
      ...
      <div class="mpn-contenido">
        <div class="mpn-header">
          ${esProfesional ? `<span class="mpn-badge mpn-badge-pro">${textoBadgePro}</span>` : `<span class="mpn-badge mpn-badge-personal">${textoBadgePersonal}</span>`}
          ${tieneCertificado ? `<span class="mpn-cert"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> ${textoCertificado}</span>` : ""}
        </div>
        ...
        <div class="mpn-acciones">
          ...
          <button class="mpn-btn-cerrar-texto">${textoCerrar}</button>
        </div>
      </div>
